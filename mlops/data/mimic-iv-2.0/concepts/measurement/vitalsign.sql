...
SELECT
    ...
    , AVG(CASE WHEN itemid IN (220180, 220051, 225310, 227242, 224643)
                AND valuenum > 0
                AND valuenum < 300
                THEN valuenum END
    ) AS dbp
    ...
    , MAX(CASE WHEN itemid = 224027 THEN value END
    ) AS skin_temperature
    ...
    , AVG(CASE WHEN itemid IN (220227)
                AND valuenum > 0
                AND valuenum <= 100
                THEN valuenum END
    ) AS sao2
...
WHERE ce.stay_id IS NOT NULL
    ...
    (
        ...
        , 220227 -- SAO2, arterial
        , 224643 -- Manual Blood Pressure Diastolic Right
        , 227242 -- Manual Blood Pressure Diastolic Left
