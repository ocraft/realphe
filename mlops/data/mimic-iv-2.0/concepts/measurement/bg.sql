...
WITH bg AS (
    SELECT
        ...
        , MAX(CASE WHEN itemid = 50821 AND valuenum >=0 THEN valuenum ELSE NULL END) AS po2
        , MAX(CASE WHEN itemid = 50822 AND valuenum<=30 THEN valuenum ELSE NULL END) AS potassium
        , MAX(
            CASE WHEN itemid = 50823 THEN valuenum ELSE NULL END
        ) AS requiredo2
        , MAX(CASE WHEN itemid = 50824 and valuenum<=200 THEN valuenum ELSE NULL END) AS sodium
        , MAX(
