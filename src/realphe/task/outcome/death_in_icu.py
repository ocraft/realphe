...
def run():
    ...
    out_path = f'{params.outcome.path.data.processed}/death_in_icu/'
...
def to_outcome(
    ...
    y_test = pd.DataFrame(np.repeat(
        cohort.sel(sample=cast(pd.MultiIndex, test.index).levels[0])['icu_expire_flag'],
        test.groupby(level=0).size(),
        axis=0
    ), index=test.index, columns=['icu_expire_flag'])

    y_oof = pd.DataFrame(np.repeat(
        cohort.sel(sample=cast(pd.MultiIndex, oof.index).levels[0])['icu_expire_flag'],
        oof.groupby(level=0).size(),
        axis=0
    ), index=oof.index, columns=['icu_expire_flag'])
...
def predict_death(
    ...
    monitor = Monitor(
        eval_path=f'{params.outcome.path.eval}/death_in_icu/eval',
        dvcyaml=None
    )
    monitor.log_params(params.outcome.death_in_icu)
    ...
    seeds = params.outcome.death_in_icu.evaluate.cv.seeds
    n_folds = params.outcome.death_in_icu.evaluate.cv.folds
    for i_seed, seed in tqdm.tqdm(enumerate(seeds)):
        set_random_seed(seed)

        splitter = create_splitter(n_folds, params.outcome.death_in_icu.evaluate.cv.shuffle, seed)
...
def train_task(
    ...
    hparams = params.death_in_icu.hparams
    ...
    live = Live(
        dir=f'{params.path.eval}/death_in_icu/train/{seed}/fold_{fold}',
        report='html',
        dvcyaml=None,
        save_dvc_exp=False
    )
...
def create_model(X_train: np.ndarray, y_train: np.ndarray, hparams: DictConfig) -> DeathInICU:
