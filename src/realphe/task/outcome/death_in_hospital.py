...
class DeathInHospital(nn.Module):
...
def run():
    ...
    out_path = f'{params.outcome.path.data.processed}/death_in_hospital/'
...
def to_outcome(
    ...
    y_test = pd.DataFrame(np.repeat(
        cohort.sel(sample=cast(pd.MultiIndex, test.index).levels[0])['hospital_expire_flag'],
        test.groupby(level=0).size(),
        axis=0
    ), index=test.index, columns=['hospital_expire_flag'])

    y_oof = pd.DataFrame(np.repeat(
        cohort.sel(sample=cast(pd.MultiIndex, oof.index).levels[0])['hospital_expire_flag'],
        oof.groupby(level=0).size(),
        axis=0
    ), index=oof.index, columns=['hospital_expire_flag'])
...
def predict_death(
    ...
    monitor = Monitor(
        eval_path=f'{params.outcome.path.eval}/death_in_hospital/eval',
        dvcyaml=None
    )
    monitor.log_params(params.outcome.death_in_hospital)
    ...
    seeds = params.outcome.death_in_hospital.evaluate.cv.seeds
    n_folds = params.outcome.death_in_hospital.evaluate.cv.folds
    for i_seed, seed in tqdm.tqdm(enumerate(seeds)):
        set_random_seed(seed)

        splitter = create_splitter(
            n_folds, params.outcome.death_in_hospital.evaluate.cv.shuffle, seed)
...
def train_task(
    ...
    hparams = params.death_in_hospital.hparams
    ...
    live = Live(
        dir=f'{params.path.eval}/death_in_hospital/train/{seed}/fold_{fold}',
        report='html',
        dvcyaml=None,
        save_dvc_exp=False
    )
...
def create_model(X_train: np.ndarray, y_train: np.ndarray, hparams: DictConfig) -> DeathInHospital:
    out_bias = focal_loss_bias_init(y_train, alpha=0.5, gamma=0.0).astype(np.float32)

    return DeathInHospital(
