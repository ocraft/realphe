...
def run():
    ...
    out_path = f'{params.outcome.path.data.processed}/death_24h/'
    ...
    y_test = to_decompensation(cohort, test)
    y_oof = to_decompensation(cohort, oof)
...
def to_decompensation(cohort: xr.Dataset, df: pd.DataFrame) -> pd.DataFrame:
    lookahead_steps = 24 // 2  # 24h / 2h = 12 steps
    stay_sizes = df.groupby(level=0).size()
    death_flags = (
        cast(pd.Series, cohort['icu_expire_flag'].to_pandas())
        .to_frame()
        .reindex(stay_sizes.index)
    )
    stay_sizes_val = cast(np.ndarray, stay_sizes.values)
    repeated_flags = np.repeat(death_flags.values, cast(np.ndarray, stay_sizes_val))
    labels = np.zeros_like(repeated_flags, dtype=int)
    starts = np.cumsum(stay_sizes_val) - lookahead_steps
    ends = np.cumsum(stay_sizes_val)
    for start, end, flag in zip(starts, ends, death_flags.values):
        if flag == 1:
            labels[start:end] = 1  # last lookahead_steps marked as 1

    return pd.DataFrame(
        labels,
        index=df.index,
        columns=['decompensation_24h']
    )
...
def predict_death(
    ...
    monitor = Monitor(
        eval_path=f'{params.outcome.path.eval}/death_24h/eval',
        dvcyaml=None
    )
    monitor.log_params(params.outcome.death_24h)
    ...
    seeds = params.outcome.death_24h.evaluate.cv.seeds
    n_folds = params.outcome.death_24h.evaluate.cv.folds
    for i_seed, seed in tqdm.tqdm(enumerate(seeds)):
        set_random_seed(seed)

        splitter = create_splitter(n_folds, params.outcome.death_24h.evaluate.cv.shuffle, seed)
    ...
    monitor.log_learning_curve(
        ['aucpr', 'aucroc', 'loss'],
        title='Outcome::24 hours Mortality::Learning Curve'
    )
    monitor.end(
        metrics(
            y_oof.values,
...
def train_task(
    ...
    hparams = params.death_24h.hparams
    ...
    live = Live(
        dir=f'{params.path.eval}/death_24h/train/{seed}/fold_{fold}',
        report='html',
        dvcyaml=None,
        save_dvc_exp=False
    )
...
def create_model(X_train: np.ndarray, y_train: np.ndarray, hparams: DictConfig) -> Death24h:
